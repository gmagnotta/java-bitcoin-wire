...
public class Sha256Hash implements Serializable, Comparable<Sha256Hash> {
	...
	private Sha256Hash(ByteBuffer byteBuffer) {
		// checkArgument(rawHashBytes.length == LENGTH);
	...
	public static Sha256Hash wrapReversed(byte[] rawHashBytes) {
		return wrap(Utils.reverseBytesClone(rawHashBytes));
	}
	 ...
	 *             if the given array length is not exactly 32
	 */
	public static Sha256Hash wrapReversed(byte[] rawHashBytes, int offset, int lenght) {
		
		return wrap(Utils.reverseBytesnoClone(Arrays.copyOfRange(rawHashBytes,  offset, offset + lenght)));
	 ...
	 * @return a new instance containing the calculated (one-time) hash
	 */
	public static Sha256Hash of(byte[] contents, int offset, int lenght) {
		return wrap(hash(contents, offset, lenght));
	}
	...
	public static Sha256Hash of(File file) throws IOException {
		...
		try {
			return of(hash(new BufferedInputStream(in)));
		} finally {
			in.close();
		}
	}
	 ...
	 * @return the hash (in big-endian order)
	 */
	public static byte[] hash(BufferedInputStream bufferedInputStream) throws IOException {
	...
	public int hashCode() {
		...
		// Bitcoin.
		ByteBuffer byteBuffer = ByteBuffer.wrap(new byte[] {bytes[LENGTH - 4], bytes[LENGTH - 3], bytes[LENGTH - 2], bytes[LENGTH - 1]});
	...
	public String toReversedString() {
		return Hex.toHexString(Utils.reverseBytesClone(bytes));
	}
	...
	public byte[] getReversedBytes() {
		return Utils.reverseBytesClone(bytes);
	}
	
	public Sha256Hash getReversed() {
		return wrapReversed(bytes);
	}
